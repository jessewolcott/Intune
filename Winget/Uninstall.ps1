...
Start-Transcript -Path "$Path_local\$PackageName-uninstall.txt" -Force 
...
IF ($PackageName){
    try {
        Write-Output "Uninstalling $($PackageName) via $PackageSource"
        ...
        .\winget.exe uninstall $PackageName --silent --accept-source-agreements # 
        Write-Output "Removing Update Script from $UpdateScriptPath."
        Remove-Item (Join-Path $UpdateScriptPath ($PackageName+$FileSuffix)) -Force
